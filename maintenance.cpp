...
maintenance::maintenance(int CIN_employe, QString reference_equipement,
                         QDate date_debut, QDate date_fin,
...
void maintenance::setReferenceEquipement(const QString &reference) {
  reference_equipement = reference;
}
...
bool maintenance::ajouter() {
  QSqlQuery query;
  query.prepare("INSERT INTO MAINTENANCE(CIN_EMPLOYE, REFERENCE_EQUIPEMENT, "
                "DATE_DEBUT, DATE_FIN, PRIX_MAINTENANCE) "
                "VALUES (:CIN_EMPLOYE, :REFERENCE_EQUIPEMENT, :DATE_DEBUT, "
                ":DATE_FIN, :PRIX_MAINTENANCE)");
