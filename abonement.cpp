...
void Abonement::setCin(const QString &c) { cin = c; }

void Abonement::setPrice(const QString &p) { price = p; }
void Abonement::setDuration(const QString &dur) { duration = dur; }

void Abonement::setMembre(const QString &mem) { membre = mem; }

QSqlQueryModel *Abonement::afficher_abonnement() {
  QSqlQueryModel *model = new QSqlQueryModel();
  model->setQuery(
      "SELECT CIN, DURATION, MEMBRE, ACTIVITY, PRICE, ID_ABNT FROM ABONNEMENT");
...
bool Abonement::ajouter() {
  QSqlQuery query;
  query.prepare(
      "INSERT INTO abonnement(id_abnt, activity, membre, cin,price,duration) "
      "VALUES (:id_abnt, :activity, :membre, :cin,:price,:duration)");
...
bool Abonement::modifier(QString id_abnt) {
  QSqlQuery query;
  query.prepare(
      "UPDATE abonnement SET cin=:cin, duration=:duration, "
      "membre=:membre,activity=:activity,price=:price WHERE id_abnt=:id_abnt");
  query.bindValue(":id_abnt", id_abnt);
  query.bindValue(":cin", cin);
  query.bindValue(":duration", duration);
  query.bindValue(":membre", membre);
