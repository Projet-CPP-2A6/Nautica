...
void stat1::make_duration()
{
    ...
    QString Nat_naaat;
    QSqlQuery q;
    q.prepare("SELECT COUNT(DURATION) FROM ABONNEMENT where DURATION ='1 mounth' ");
            q.exec();
            q.first() ;
            nat_tn=(q.value(0).toInt());
    q.prepare("SELECT COUNT(DURATION) FROM ABONNEMENT where DURATION ='3 mounths' ");
            q.exec();
            q.first() ;
            nat_fr=(q.value(0).toInt());
    q.prepare("SELECT COUNT(DURATION) FROM ABONNEMENT where DURATION ='9 mounths' ");
                    q.exec();
                    q.first() ;
