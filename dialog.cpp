...
Dialog::Dialog(QWidget *parent) : QDialog(parent), ui(new Ui::Dialog) {
  ...
  /*
   *  Testing code, prints the description, vendor id, and product id of all
  ports.
  ...
  QSerialPortInfo::availablePorts()){ qDebug() << "Description: " <<
  ...
  */
  ...
  foreach (const QSerialPortInfo &serialPortInfo,
    ...
    //  check if the serialport has both a product identifier and a vendor
    //  identifier
  ...
  if (arduino_is_available) {
    ...
    QMessageBox::information(this, "Serial Port Error",
                             "Couldn't open serial port to arduino.");
  }
}
...
void Dialog::readSerial() {
  /*
   * readyRead() doesn't guarantee that the entire message will be received all
   * at once. The message can arrive split into parts.  Need to buffer the
   * serial data and then parse for the temperature value.
   *
   */
  ...
  //  means there is a parsed temperature value as the second token (between 2
  //  commas)
  if (buffer_split.length() < 3) {
    ...
    // the second element of buffer_split is parsed correctly, update the
    // temperature value on temp_lcdNumber
    serialBuffer = "";
    qDebug() << buffer_split << "\n";
    parsed_data = buffer_split[1];
    temperature_value =
        (9 / 5.0) * (parsed_data.toDouble()); // convert to fahrenheit

    // format precision of temperature_value to 4 digits or fewer
    Dialog::updateTemperature(parsed_data);
    QSqlQuery query;
    query.prepare(
        "INSERT INTO arduino(temperature_value) VALUES (:temperature_value)");
