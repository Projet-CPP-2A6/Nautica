...
Equipements::Equipements(int reference, float prix, int nombre,
...
// getters
int Equipements::getReference() { return reference; };
float Equipements::getPrix() { return prix; };
...
QString Equipements::getImage() { return image; };

// setters
void Equipements::setReference(int reference) { this->reference = reference; };
void Equipements::setPrix(float prix) { this->prix = prix; };
...
void Equipements::setImage(QString image) { this->image = image; };
...
bool Equipements::ajouter() {
  ...
  query.prepare(
      "INSERT INTO "
      "Equipements(REFERENCE,PRIX,NOMBRE,FONCTIONALITE,TYPE,ETAT,IMAGE)"
...
bool Equipements::modifier() {
  ...
  query.prepare("UPDATE EQUIPEMENTS SET REFERENCE=:REFERENCE, PRIX=:PRIX, "
                ...
                "IMAGE=:IMAGE WHERE REFERENCE = :REFERENCE ");
  query.bindValue(":REFERENCE", reference);
  query.bindValue(":PRIX", prix);
  query.bindValue(":NOMBRE", nombre);
  query.bindValue(":FONCTIONALITE", fonctionalite);
...
QSqlQueryModel *Equipements::afficher() {
  ...
  model->setQuery("SELECT REFERENCE,PRIX,NOMBRE,FONCTIONALITE,TYPE,ETAT,IMAGE "
                  "FROM EQUIPEMENTS");
